...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event(), static_dir=None):
    ...
    web_root.putChild('recent_blocks', WebInterface(lambda: [dict(
        ...
        share='%064x' % s.hash,
    ) for s in node.tracker.get_chain(node.best_share_var.value, min(node.tracker.get_height(node.best_share_var.value), 10*1*1//node.net.SHARE_PERIOD)) if s.pow_hash <= s.header['bits'].target]))
    ...
    def update_stat_log():
        while stat_log and stat_log[0]['time'] < time.time() - 10*1*1:
            stat_log.pop(0)
    ...
    dataview_descriptions = {
        ...
        'last_day': graph.DataViewDescription(300, 10*1*1),
        'last_week': graph.DataViewDescription(300, 10*1*1*7),
        'last_month': graph.DataViewDescription(300, 10*1*1*30),
        'last_year': graph.DataViewDescription(300, 10*1*1*365.25),
    }
